...
export class UserService {
  ...
  async updateData(req, updateUserDto: UpdateUserDto) {
    ...
    if (user._id.toString() !== req.user.id) throw new BadRequestException('You cant update this account data because this not your account')

    const { firstName, lastName, age, phoneNumber } = updateUserDto
    const userInPhonenumber = await this.userModel.findOne({ phoneNumber })
    if (userInPhonenumber) throw new BadRequestException(`${phoneNumber} - has arleady`)

    await this.userModel.findByIdAndUpdate(req.user.id, { firstName, lastName, age, phoneNumber })
  ...
  async updatePassword(req, updateUserPassword: UpdtaeUserPassword) {
    const user = await this.userModel.findById(req.user.id);
    if (!user) {
      throw new BadRequestException('You can\'t update this account password because this is not your account');
    }
    ...
    const hashedPassword = bcrypt.hashSync(confirmPassword, bcrypt.hashSync(confirmPassword, 10));
  ...
  async forgotPassword(email: string) {
    ...
    this.emailService.sendMail("hammkrtchyan7@gmail.com", "Enter this code to verify your identity", message)
  ...
  async deleteAccount(req) {
    const user = await this.userModel.findById(req.user.id)
    if (!user) throw new BadRequestException('User not found')
    if (user._id.toString() !== req.user.id) throw new BadRequestException('You cant delete this account because this not your account')
    ...
    const feedback = await this.feedbackModel.findOne({ customer: req.user.id })
