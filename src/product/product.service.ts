...
export class ProductService {
  ...
  async findAll() {
    const products = await this.productModel.find({ status: 1 }).populate('subCategory');
  ...
  async findMyAllProducts(req) {
    const products = await this.productModel.find({ manager: req.user.id }).populate('subCategory');
    if (!products.length) throw new NotFoundException('There are no products')
    return products;
  }
  ...
  async myRejectedProds(req) {
    const products = await this.productModel.find({ manager: req.user.id, status: -1 })
  ...
  async findOne(id: string) {
    const product = await this.productModel.findOne({ id, status: 1 }).populate('subCategory')
  ...
  async updateData(id: string, updateProductData: UpdateProductData, req) {
    ...
    if (req.user.id !== product.manager.toString()) throw new BadRequestException('You cannot update this product because you did not install it')

    await this.productModel.findByIdAndUpdate(id, { name, price, count, description })
    return product
  }
  ...
  async addImage(id: string, files, req) {
    ...
    if (!product) throw new BadRequestException('Product not found')
    if (req.user.id !== product.manager.toString()) throw new BadRequestException('You cannot add image in this product because you did not install it')
  ...
  async updateSubCategory(id: string, updateSubCategory: UpdateSubCategory, req) {
    const product = await this.productModel.findById(id)

    if (!product) throw new BadRequestException('Product not found')
    if (req.user.id !== product.manager.toString()) throw new BadRequestException('You cannot update sub category in this product because you did not install it')

    const { subCategory } = updateSubCategory
    const subCat = await this.subCategoryModel.findOne({ _id: subCategory });
  ...
  async deleteImage(id, imageName, req) {
    const product = await this.productModel.findById(id)
    if (!product) throw new BadRequestException('Product not found')
    if (req.user.id !== product.manager._id.toString()) throw new BadRequestException('You cannot delete this product image because you did not install that product')
  ...
  async remove(id: string, req) {
    const product = await this.productModel.findById(id)
    if (req.user.id !== product.manager._id.toString()) throw new BadRequestException('You cannot delete this product because you did not install that product')
